...
def filter_paths_for_only_foreground_parallel(data_paths, dataset="label_crop/all", n_workers=4):
...
class AtLeastNGroupsSampler:
    ...
    def __call__(self, x: np.ndarray, y: np.ndarray) -> bool:
        ...
        satisfied_groups, satisfied_instances = 0, 0
        for group in self.id_groups:
            mask = np.isin(y, group)
            if self.min_size is not None:
                present_ids = [i for i in group if np.sum(y == i) >= self.min_size]
                ...
                n_found = np.unique(y[mask]).size
            if n_found > 0:
                satisfied_instances += n_found
                satisfied_groups += 1
        if len(self.id_groups) == 1 and np.sum(y == 0) == 0:
            # If there is only one group and no background, reject
            return np.random.rand() > self.p_reject
        if satisfied_groups >= self.min_num_groups or satisfied_instances >= self.min_num_instances:
            # Not enough groups satisfied, reject
            return True
        # print("satisfied_groups", satisfied_groups, "satisfied_instances", satisfied_instances)
        return np.random.rand() > self.p_reject
